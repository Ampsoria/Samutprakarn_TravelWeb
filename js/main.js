document.addEventListener('DOMContentLoaded', () => {
    ...
    if (showLoginBtn && showRegisterBtn && loginForm && registerForm) {
        // เช็กค่าจาก URL กรณีถูกส่งมาจากปุ่มสมัครสมาชิก
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadProfile() {
        ...
        if (displayName && displayTags && displayAvatar) {
            try {
                ...
                displayName.textContent = user.fullName || 'ไม่มีชื่อ';
                displayAvatar.textContent = (user.fullName || '?').charAt(0).toUpperCase();

                // Render tags
                const tagsArray = (user.interests || '').split(',').map(t => t.trim()).filter(t => t);
                if (tagsArray.length > 0) {
                    displayTags.innerHTML = tagsArray.map(t => `<span class="badge" style="margin-right:4px;">${t}</span>`).join('');
                } else {
                    displayTags.innerHTML = '<span class="text-muted text-small">ไม่มีข้อมูลความสนใจ</span>';
                }

                // Update stats
                const statItems = document.querySelectorAll('.profile-stats .h3');
                if (statItems.length >= 2) {
                    statItems[0].textContent = user.reviewCount || 0;
                    statItems[1].textContent = user.bookmarkCount || 0;
                }
            } catch (err) {
    ...
    // --- Review Management ---
    // Reviews are now loaded via ReviewsAPI in the detail page rendering section
    ...
    if (writeReviewForm) {
        writeReviewForm.addEventListener('submit', async (e) => {
            ...
            const btn = writeReviewForm.querySelector('button[type="submit"]');
            if (btn) btn.disabled = true;
            ...
            try {
                if (!AuthAPI.isLoggedIn()) {
                    alert('กรุณาเข้าสู่ระบบก่อนเขียนรีวิว / Please login first');
                    return;
                }
                await ReviewsAPI.create(placeId, parseInt(rating), comment);
                closeReviewModal();
                document.getElementById('reviewRating').value = '5';
                document.getElementById('reviewComment').value = '';

                const lang = localStorage.getItem('appLang') || 'th';
                alert(lang === 'en' ? 'Review submitted for moderation!' : 'ส่งรีวิวเพื่อตรวจสอบแล้ว!');
            } catch (err) {
                alert(err.message || 'เกิดข้อผิดพลาด / Error');
            } finally {
                if (btn) btn.disabled = false;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (adminPlacesList) {
        ...
        window.loadAdminPlaces = async () => {
            ...
            if (places.length === 0) {
                adminPlacesList.innerHTML = '<p style="padding:16px; text-align:center;">ไม่พบสถานที่</p>';
                return;
            }
            let html = '';
            places.forEach(p => {
                ...
                <div class="place-item">
                    <img src="${p.img}" class="place-item-img">
                    <div class="place-item-info">
                        <div class="flex items-center gap-2 mb-1">
                            <h3 style="font-size:1.1rem; margin:0;" data-i18n="place_dynamic">${p.nameTh}</h3>
        ...
        const addPlaceForm = document.getElementById('adminAddPlaceForm');
        if (addPlaceForm) {
            addPlaceForm.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    alert('เพิ่มสถานที่สำเร็จ / Place added successfully');
                    loadAdminPlaces();
                    addPlaceForm.reset();
                } catch (err) {
    ...
    // --- 3. Manage Reviews (via API) ---
    const adminReviewsList = document.getElementById('adminReviewsList');
    if (adminReviewsList) {

        window.loadAdminReviews = async (statusFilter) => {
            let reviews = [];
            try {
                const data = await ReviewsAPI.getAdminReviews(statusFilter || 'pending');
            ...
            reviews.forEach(r => {
                ...
                const placeName = r.place ? r.place.nameTh : 'N/A';
                const userName = r.user ? (r.user.fullName || r.user.email) : 'N/A';
                const dateStr = r.createdAt ? new Date(r.createdAt).toLocaleDateString('th-TH') : '';
                html += `
                <div class="review-admin-card">
                    ...
                    <div class="actions">
                        ${r.status === 'pending' ? `<button class="btn btn-primary" onclick="approveReview(${r.id})">✅ อนุมัติ</button>` : ''}
        ...
        window.approveReview = async (id) => {
            if (confirm('อนุมัติรีวิวนี้หรือไม่? / Approve?')) {
                try {
                    await ReviewsAPI.moderate(id, 'approved');
                    loadAdminReviews();
                } catch (err) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (detailContentContainer) {
        ...
        if (place) {
            ...
            if (mapBtn) {
                const mapUrl = place.googleMapsUrl || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(place.nameTh + ' สมุทรปราการ')}`;
                mapBtn.href = mapUrl;
            }
    ...
    // --- Profile Bookmarks Rendering (via API) ---
    const profileBookmarksList = document.getElementById('profileBookmarksList');
    if (profileBookmarksList) {
        window.loadProfileBookmarks = async () => {
            if (!AuthAPI.isLoggedIn()) {
                profileBookmarksList.innerHTML = '<p style="padding:16px; text-align:center; color:var(--text-muted); background:white; border-radius:8px; border:1px solid #E5E7EB;">กรุณาเข้าสู่ระบบเพื่อดูรายการโปรด</p>';
                return;
            }
            ...
            bookmarks.forEach(b => {
                ...
                html += `
                    <div class="list-card">
                        <img src="${imgUrl}" class="list-img" alt="${b.nameTh}" onerror="handleImgError(this)">
                        <div class="list-body">
                            <div class="flex justify-between items-center w-100">
                                <span class="badge mb-1" data-i18n="badge_${b.category}">หมวดหมู่</span>
                                <button class="remove-btn" title="ลบออกจากรายการโปรด" onclick="removeBookmark(${b.placeId})">
                                    <svg width="20" height="20" fill="currentColor" viewBox="0 0 20 20">
        ...
        window.removeBookmark = async (placeId) => {
            try {
                await BookmarksAPI.remove(placeId);
                loadProfileBookmarks();
            } catch (err) {
                alert(err.message || 'Error removing bookmark');
            }
        };
